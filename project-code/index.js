...
app.post("/login", async (req, res) => {
  ...
  db.one(query, [req.body.username])
    ...
    .catch((err) => {
      ...
      return res.render("./pages/login", {
        error: true,
        message: "No account is associated with that username.",
      });
    });
});
...
app.post("/register", async (req, res) => {
  if (req.body.password !== req.body.confirmPassword) {
    return res.render("./pages/login", {
      error: true,
      message: "Failed to register, passwords did not match. Try again.",
    });
  }
  ...
  const insertQuery =
    "INSERT INTO players (username, playerName, password, classYear, profilePhoto, joinDate, email, phone, gender) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);";
  db.any(insertQuery, [
    req.body.username,
    req.body.playername,
    hash,
    classYear,
    req.body.profilePhotoLink,
    joinDate,
    email,
    req.body.phone,
    gender,
    ...
    .catch((err) => {
      ...
      res.render("./pages/login", {
        message:
          "Failed to register, that username or email is already taken. Try again.",
        error: 1,
      });
    });
});
    ...
    .then((rows) => {
      ...
      let transporter = nodemailer.createTransport({
        ...
        auth: {
          ...
          pass:
            process.env.EMAIL_PWD_MAC ||
            process.env.EMAIL_PWD_WINDOWS ||
            process.env.EMAIL_PWD_IPHONE ||
            process.env.EMAIL_PWD_IPAD,
        },
        tls: {
...
app.post("/resetPassword", async (req, res) => {
  ...
  if (req.body.newPassword !== req.body.confirmNewPassword) {
    ...
    db.one(
      `UPDATE players SET password = '${hash}' WHERE email = '${req.body.email}' RETURNING playerID;`
    )
      .then((playerID) => {
...
app.get("/teams/:sportName", (req, res) => {
  ...
  db.tx((t) => {
    ...
    const teams = t.any(
      "SELECT * FROM teams WHERE teamID IN (SELECT teamID FROM teamsToSports WHERE sportID = (SELECT sportID FROM sports WHERE sportName = $1));",
      [sportName]
    );
    const sportID = t.one("SELECT sportID FROM sports WHERE sportName = $1;", [
      sportName,
    ]);
    ...
    .catch((err) => {
      ...
      return res.render("./pages/sports", {
        message: "Error Occured In Team Query",
        error: 1,
        user: req.session.user,
      });
    });
...
app.get("/team/:teamID", (req, res) => {
  db.tx((t) => {
    // creating a sequence of transaction queries:
    const sport = db.any(
      `SELECT sportName FROM sports WHERE sportID IN (SELECT sportID FROM teamsToSports WHERE teamID =  ${req.params.teamID} );`
    );
    const onTeam = db.any(
      `SELECT * FROM teamsToPlayers WHERE playerID = ${req.session.user.playerid} AND teamID = ${req.params.teamID};`
    );
    const team = db.one("SELECT * FROM teams WHERE teamID=$1", [
      req.params.teamID,
    ]);
    const teamsToCaptains = db.one(
      "SELECT * FROM teamsToCaptains WHERE teamID=$1",
      [req.params.teamID]
    );
    const players = db.any(
      `SELECT *  FROM players 
    INNER JOIN teamsToPlayers ON players.playerID = teamsToPlayers.playerID
    WHERE teamsToPlayers.teamID = $1`,
      [req.params.teamID]
    );
    const teamGames = db.any(
      `SELECT *  FROM games 
    INNER JOIN teamsToGames ON games.gameID = teamsToGames.gameID
    WHERE teamsToGames.teamID = $1 ORDER BY games.gameDate `,
      req.params.teamID
    );
    const numWinners = db.one(
      `SELECT COUNT(*) FROM gamesToWinners WHERE teamID = $1;`,
      req.params.teamID
    );
    const gamesQuery = t.any(
      `SELECT * FROM games INNER JOIN teamsToGames ON games.gameID = teamsToGames.gameID
     INNER JOIN teamsToSports ON teamsToGames.teamID = teamsToSports.teamID
      INNER JOIN sports ON teamsToSports.sportID = sports.sportID
       ...
       (SELECT gameid FROM teamsToGames WHERE teamsToGames.teamID = $1) ORDER BY games.gameDate;`,
      req.params.teamID
    );
    // returning a promise that determines a successful transaction:
    return t.batch([
    ...
    .then((data) => {
      if (data.length == 1) {
        ...
        if (req.body.iscaptain == "true") {
          db.any(
            `SELECT playerID FROM teamsToPlayers WHERE teamID = ${req.body.teamid} AND playerID != ${req.session.user.playerid} LIMIT 1;`
          )
            .then((newCaptainID) => {
              if (newCaptainID.length > 0) {
                db.any(
                  `DELETE FROM teamsToPlayers WHERE teamID = ${req.body.teamid} AND playerID = ${req.session.user.playerid}; 
...
app.get("/team/delete/:teamID", async (req, res) => {
  ...
  try {
    await db.one(checkTeamCaptain, [
      req.params.teamID,
      req.session.user.playerid,
    ]);
  } catch (err) {
    console.log(err);
    return res.redirect("/yourTeams");
  ...
  `
    ...
    .then((data) => {
      if (data.length > 0) {
        ...
        db.any(
          `SELECT * FROM teamsToPlayers WHERE playerID = ${req.session.user.playerid} AND teamID = ${req.body.teamid};`
        )
          .then((data) => {
            if (data.length > 0) {
              return res.redirect(
                "./" + req.body.teamid + "?joinResult=onTeamError"
              );
            } else {
              const query = `INSERT INTO teamsToPlayers (playerID, teamID) VALUES ($1, $2);`;
              db.any(query, [req.session.user.playerid, req.body.teamid])
                .then(() => {
                  db.one("SELECT * FROM teams WHERE teamID=$1", [
                    req.body.teamid,
                  ])
                    .then((team) => {
                      console.log("team:" + team.teamname);
                      return res.redirect(
                        "./" + req.body.teamid + "?joinResult=success"
                      );
                    })
                    .catch((err) => {
                      console.log(err);
...
app.post("/team/create", (req, res) => {
  ...
  const teamInsertQuery =
    "INSERT INTO teams (teamName) VALUES ($1) RETURNING teamID";
  ...
  db.any(teamInsertQuery, [req.body.teamName])
    .then((teamID) => {
      db.none(teamRelationInsertQuery, [
        req.session.user.playerid,
        teamID[0].teamid,
        sportID,
      ])
        ...
        .catch((err) => {
          ...
          return res.render(`./pages/sports`, {
            error: true,
            message: "Unable to add team relations",
            user: req.session.user,
          });
        });
    })
...
app.post("/allGames/create", (req, res) => {
  const gameInsertQuery =
    "INSERT INTO games (gameDate, time, location) VALUES ($1,$2,$3) RETURNING gameID";
  ...
  db.any(gameInsertQuery, [
    req.body.gameDate,
    req.body.gameTime,
    req.body.gameLocation,
  ])
    .then((data) => {
      db.none(gameRelationInsertQuery, [
        data[0].gameid,
        req.body.teamid1,
        req.body.teamid2,
      ])
        ...
        .catch((err) => {
          ...
          return res.render(`./pages/createSuccess`, {
            ...
            playerID: req.session.user.playerid,
            error: true,
            message: "Unable to add game relations.",
          });
        });
    })
    .catch((err) => {
...
app.post("/editProfile", (req, res) => {
  ...
  db.one(
    "UPDATE players SET username = $1, playerName = $2, classYear = $3, gender = $8, profilePhoto = $4, email = $5, phone = $6 WHERE playerID = $7 RETURNING playerID;",
    [
      req.body.username,
      req.body.playername,
      year,
    ...
    .catch((err) => {
      ...
      return res.render("pages/profile", {
        ...
        error: true,
        message:
          "Error. That username or email is already registered with another account.",
      });
    });
});
...
app.get("/yourUpcomingGames", (req, res) => {
  db.tx((t) => {
    const teamsQuery = t.any(
      `SELECT * FROM teamsToPlayers WHERE playerid = ${req.session.user.playerid};`
    );
    const gamesQuery = t.any(
      `SELECT * FROM games INNER JOIN teamsToGames ON games.gameID = teamsToGames.gameID INNER JOIN teamsToSports ON teamsToGames.teamID = teamsToSports.teamID INNER JOIN sports ON teamsToSports.sportID = sports.sportID INNER JOIN teams ON teamsToSports.teamID = teams.teamID WHERE games.gameid IN (SELECT gameid FROM teamsToGames WHERE teamsToGames.teamID IN (SELECT teamid FROM teamsToPlayers WHERE playerid = ${req.session.user.playerid})) ORDER BY games.gameDate;`
    );
    ...
    .then((data) => {
      ...
      return res.render("./pages/yourUpcomingGames", {
        teams: data[0],
        games: gameData,
        user: req.session.user,
      });
...
app.get("/allUpcomingGames", (req, res) => {
  ...
  db.any(query)
    .then((data) => {
      ...
      return res.render("./pages/allUpcomingGames", {
        games: gameData,
        user: req.session.user,
      });
    })
...
app.get("/allGames", (req, res) => {
  db.tx((t) => {
    ...
    const gamesQuery = t.any(
      `SELECT * FROM games INNER JOIN teamsToGames ON games.gameID = teamsToGames.gameID INNER JOIN teamsToSports ON teamsToGames.teamID = teamsToSports.teamID INNER JOIN sports ON teamsToSports.sportID = sports.sportID INNER JOIN teams ON teamsToSports.teamID = teams.teamID ORDER BY games.gameDate;`
    );
...
app.get("/game", (req, res) => {
  ...
  db.any(
    `SELECT teamname, teamid FROM teams WHERE teamid IN(SELECT teamid FROM teamsToGames WHERE gameID = $1 LIMIT 2);`,
    [req.query.gameid]
  )
    .then(function (teaminfo) {
      db.any(`SELECT * FROM games WHERE gameID = $1`, [req.query.gameid])
        .then(function (game) {
          db.any(
            `SELECT sportname FROM sports WHERE sportid = (SELECT sportid FROM teamsToSports WHERE teamid = ${teaminfo[0].teamid} LIMIT 1);`
          )
...
app.get("/yourTeams", (req, res) => {
  db.any(
    `SELECT * FROM teams WHERE teamID IN (SELECT teamID FROM teamsToPlayers WHERE playerID = ${req.session.user.playerid});`
  )
    .then((teams) => {
...
app.get("/searchPlayers", (req, res) => {
  ...
  db.any(
    ...
    .then((players) => {
      if (players.length == 0) {
        db.any(
          `SELECT * FROM players WHERE classYear = ${q} OR playerid = ${q};`
        )
          .then((playersInt) => {
...
app.get("/player", (req, res) => {
  db.any(
    `SELECT * FROM players WHERE playerid = ${req.query.playerid} LIMIT 1;`
  )
    .then((player) => {
      db.any(
        `SELECT * FROM teams WHERE teamID IN (SELECT teamID FROM teamsToPlayers WHERE playerID = ${player[0].playerid});`
      )
        .then((teams) => {
...
app.post("/changePassword", async (req, res) => {
  ...
  db.one(
    `UPDATE players SET password = '${newHash}' WHERE playerID = ${req.session.user.playerid} RETURNING playerID;`
  )
    .then((playerID) => {
      db.one("SELECT * FROM players WHERE playerID = $1", [playerID.playerid])
        .then((user) => {
...
app.use((req, res, next) => {
  res.status(404).send('<div style="text-align: center;"><h4>404</h4></div>');
});
